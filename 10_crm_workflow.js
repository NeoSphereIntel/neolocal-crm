...
function refreshCRMExecutionLayer() {
  ...
  rows.forEach(row => {
    ...
    if (obj.next_action_due_at) {
      const due = new Date(obj.next_action_due_at);
      obj.is_overdue = (due < stripTime_(now) && !isClosed_(obj.pipeline_stage)) ? "YES" : "NO";
    }

    // LAST CONTACT
    obj.last_contact_at = obj.last_reply_at || obj.last_outreach_at || obj.last_contact_at || "";

    // REPLY HANDLING (using your existing fields)
